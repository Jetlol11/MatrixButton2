...
uint8_t num = 0;
uint8_t id[11] = { 6, 4, 3, 4, 0, 5, 0, 0, 0, 1, 0 };
uint8_t assign[11] = { 0 };
...
int main(void) {
	...
	while (1) {
		...
		if (HAL_GetTick() >= timestamp) {
			...
			if (lastclick == 0 && ButtonMatrix > 0) {
				Readtonum();
			}
			lastclick = ButtonMatrix;
		}
	}
	/* USER CODE END 3 */
}
...
void Readtonum() {
	if (i < 11) {
		...
		if (assign[0] == 6 && assign[1] == 4 && assign[2] == 3 && assign[3] == 4
				&& assign[4] == 0 && assign[5] == 5 && assign[6] == 0
				&& assign[7] == 0 && assign[8] == 0 && assign[9] == 1
				&& assign[10] == 0) {
